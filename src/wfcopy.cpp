...
INT_PTR ReplaceDlgProc( HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam);
...
JAPANEND
    ...
    // Save it away.
    //
    StrNCpy(szTemp, lpszPath, std::size(szTemp));
    CheckSlashes(szTemp);
    StripColon(szTemp);
   ...
   BOOL bModal)
{
   ...
   if (aDriveInfo[drive].iBusy) {
      ...
      LoadStringW(hAppInstance, aDriveInfo[drive].iBusy, szTemp, std::size(szTemp));
      LoadStringW(hAppInstance, IDS_COPYERROR + FUNC_SETDRIVE, szTitle,
         std::size(szTitle));

      wsprintf(szMessage, szTemp, drive + CHAR_A);
      MessageBoxW(hwnd, szMessage, szTitle, MB_ICONHAND);
...
Retry:
   ...
   hCursor = LoadCursorW(NULL, IDC_WAIT);
...
DiskNotThere:
   ...
   case ERROR_NOT_READY:
      ...
      //
      LoadStringW(hAppInstance, IDS_COPYERROR + dwFunc, szTitle, std::size(szTitle));
      LoadStringW(hAppInstance, IDS_DRIVENOTREADY, szTemp, std::size(szTemp));
      wsprintf(szMessage, szTemp, drive + CHAR_A);
      if (MessageBoxW(hwnd, szMessage, szTitle, MB_ICONEXCLAMATION | MB_RETRYCANCEL) == IDRETRY)
         goto Retry;
      else
         return 0;
   ...
   case ERROR_UNRECOGNIZED_VOLUME:              // 0x1F  huh?  What's this?
      ...
      if (!CancelInfo.hCancelDlg && IsRemovableDrive(drive)) {
         LoadString(hAppInstance, IDS_UNFORMATTED, szTemp, std::size(szTemp));
         wsprintf(szMessage, szTemp, drive + CHAR_A);

         if (MessageBoxW(hwnd, szMessage, szTitle, MB_ICONEXCLAMATION| MB_YESNO) == IDYES) {
            ...
            CancelInfo.Info.Format.fmMediaType = static_cast<FMIFS_MEDIA_TYPE>(-1);       // no premedia selection
      ...
      FormatError(TRUE, szTemp, std::size(szTemp), ERROR_UNRECOGNIZED_VOLUME);
      MessageBoxW(hwnd, szTemp, szTitle, MB_OK | MB_ICONSTOP);
   ...
   LoadString(hAppInstance, IDS_COPYERROR + dwFunc, szTitle, std::size(szTitle));

   FormatError(TRUE, szTemp, std::size(szTemp), dwError);
   ...
   case WM_INITDIALOG:
      {
         ...
         if (lpdlgparams->bWriteProtect) {
            LoadString(hAppInstance, IDS_WRITEPROTECTFILE, szMessage, std::size(szMessage));
            SetDlgItemText(hDlg, IDD_STATUS, szMessage);

            LoadString(hAppInstance, IDS_ALLFILES, szMessage, std::size(szMessage));
   ...
   BOOL *pbReadOnlyAll)
{
   ...
   if ( CONFIRMNOACCESS == dlg || CONFIRMNOACCESSDEST == dlg) {
      ...
      nRetVal = DialogBoxParamW(hAppInstance, (LPTSTR) MAKEINTRESOURCE(dlg), hDlg, ReplaceDlgProc, (LPARAM)(LPPARAM_REPLACEDLG)&params);
   }
...
NetCheck(LPTSTR pPath, DWORD dwType)
{
   ...
   WNetGetLastError(&err, szT, std::size(szT), szProvider, std::size(szProvider));

   LoadString(hAppInstance, IDS_NETERR, szTitle, std::size(szTitle));
   ...
   LPARAM lParam)
{
    ...
    p = FindFileName(pszDialogFrom);
        ...
        SendDlgItemMessage(hwnd,IDD_TO,EM_LIMITTEXT,std::size(szT) - 1, 0L);
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            GetDlgItemText(hwnd,IDD_TO,szT,std::size(szT));
...
BeginDirSearch:
            ...
            //
			PathCchAppend(pcr->sz, std::size(pcr->sz), szStarDotStar);
...
ProcessSearchResult:
         ...
         if (pDTA->fd.dwFileAttributes & ATTR_DIR) {
            ...
            dwOp = OPER_MKDIR;
            RemoveLast (pcr->sz);
			PathCchAppend(pcr->sz, std::size(pcr->sz), pDTA->fd.cFileName);
            //AppendToPath (pcr->sz,pDTA->fd.cFileName);
			PathCchAppend(pcr->szDest, std::size(pcr->szDest), pDTA->fd.cFileName);
         ...
         //
         pcr->fRecurse = FALSE;
         pcr->pSource = GetNextFile (pcr->pSource,pcr->sz,std::size(pcr->sz));
...
CheckMultiple( LPTSTR pInput)
{
  ...
  pT = GetNextFile(pInput, szTemp, std::size(szTemp));
  if (!pT)
      return 0;     // blank string
  PathCchRemoveBackslash(szTemp, std::size(szTemp));
  //StripBackslash(szTemp);

  if (IsDirectory(szTemp))
      return 2;     // directory

  pT = GetNextFile(pT, szTemp, std::size(szTemp));
...
Notify(HWND hDlg, WORD idMessage, LPTSTR szFrom, LPTSTR szTo)
{
   ...
   if (idMessage) {
      LoadString(hAppInstance, idMessage, szTemp, std::size(szTemp));
...
IsWindowsFile(LPTSTR szFileOEM)
{
   ...
   auto hMod = GetModuleHandleW(szModule);
   ...
   GetModuleFileNameW(hMod, szModule, std::size(szModule));
...
WF_CreateDirectory(HWND hwndParent, LPTSTR szDest, LPTSTR szSrc)
{
   ...
   //
   StrNCpy(szTemp, szDest, std::size(szTemp)-1);
...
static VOID   WFMoveCopyDriverThread(PCOPYINFO pCopyInfo);
DWORD
WFMoveCopyDriver(PCOPYINFO pCopyInfo)
{
   ...
   try {
	   ...
	   std::thread thread(WFMoveCopyDriverThread, pCopyInfo);
	   thread.detach();
   }
...
WFMoveCopyDriverThread(PCOPYINFO pCopyInfo)
{
   ...
   if (pCopyInfo->dwFunc != FUNC_DELETE) {
      ...
      pSpec = GetNextFile(pCopyInfo->pTo, szTemp, std::size(szTemp));
      ...
      if (pCopyInfo->dwFunc == FUNC_RENAME) {
         ...
         if (!IsWild(pCopyInfo->pTo) && (ManySource || IsDirectory(pCopyInfo->pTo))) {
			 PathCchAppendEx(pCopyInfo->pTo, 2 * MAXPATHLEN, szStarDotStar, PATHCCH_ALLOW_LONG_PATHS | PATHCCH_FORCE_ENABLE_LONG_NAME_PROCESS);
...
TRY_COPY_AGAIN:
      ...
      if (pCopyInfo->dwFunc != FUNC_DELETE) {
         ...
         if (oper == OPER_DOFILE && !(bSameFile && bDoMoveRename)) {
	       ...
	       GetShortPathNameW(szDest, szShortDest, std::size(szShortDest));

	       GetShortPathNameW(szSource, szShortSource, std::size(szShortSource));
...
ExitLoop:
   ...
   if (bDirNotEmpty && !pCopyInfo->bUserAbort) {
      ...
      LoadStringW( hAppInstance, IDS_COPYMOVENOTCOMPLETED, szTitle, std::size( szTitle ));
      LoadStringW( hAppInstance, IDS_DIRREMAINS, szMessage, std::size (szMessage));

      MessageBoxW ( hdlgProgress, szMessage, szTitle, MB_ICONSTOP );
   }
   ...
   BOOL bCopy)
{
   ...
   if (bConfirmMouse) {
      ...
      LoadString(hAppInstance, IDS_MOUSECONFIRM, szTitle, std::size(szTitle));
...
Error:
      ...
      FormatError(TRUE, szMessage, std::size(szMessage), GetLastError());
      LoadString(hAppInstance, IDS_WINFILE, szTitle, std::size(szTitle));
   ...
   BOOL bFatalError)
{
   ...
   StrCpyN(szFile, bErrorOnDest ? pszDest : pszSource, std::size(szFile));
   hDC = GetDC(NULL);
   ...
   if (nOper == OPER_DOFILE || !nOper) {
      ...
      if (bErrorOnDest)
         ...
         LoadString(hAppInstance, IDS_REPLACING, szVerb, std::size(szVerb));
      else
         LoadString(hAppInstance, IDS_VERBS + dwFunc, szVerb, std::size(szVerb));

   } else {
      LoadString(hAppInstance, IDS_ACTIONS + (nOper >> 8), szVerb, std::size(szVerb));
   }

   // get the reason string

   FormatError(TRUE, szReason, std::size(szReason), dwError);
...
CopyMoveRetry(LPTSTR pszDest, INT nError, PBOOL pbErrorOnDest)
{
   ...
   do {     // until the destination path has been created
      ...
      *pbErrorOnDest = FALSE;
      ...
      GetWindowText(hdlgProgress, szTitle, std::size(szTitle));

      if (nError == ERROR_PATH_NOT_FOUND) {

         LoadString(hAppInstance, IDS_PATHNOTTHERE, szReason, std::size(szReason));
         ...
         *pTemp = CHAR_BACKSLASH;
         ...
         LoadString(hAppInstance, IDS_DESTFULL, szMessage, std::size(szMessage));
      }
