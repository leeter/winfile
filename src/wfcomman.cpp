...
extern "C" VOID(*lpfnFormat)(PWSTR, FMIFS_MEDIA_TYPE, PWSTR, PWSTR, BOOLEAN, FMIFS_CALLBACK);
extern "C" VOID(*lpfnDiskCopy)(PWSTR, PWSTR, BOOLEAN, FMIFS_CALLBACK);
extern "C" BOOLEAN(*lpfnSetLabel)(PWSTR, PWSTR);
extern "C" BOOLEAN(*lpfnQuerySupportedMedia)(PWSTR, PFMIFS_MEDIA_TYPE, DWORD, PDWORD);
...
FmifsLoaded()
{
   ...
   if (hfmifsDll < (HANDLE)32) {
	  ...
	  if (hfmifsDll < (HANDLE)32) {
		 ...
		 lpfnFormat = (decltype(lpfnFormat))GetProcAddress(hfmifsDll, "Format");
		 lpfnQuerySupportedMedia = (decltype(lpfnQuerySupportedMedia))GetProcAddress(hfmifsDll, "QuerySupportedMedia");

		 lpfnSetLabel = (decltype(lpfnSetLabel))GetProcAddress(hfmifsDll, "SetLabel");
		 lpfnDiskCopy = (decltype(lpfnDiskCopy))GetProcAddress(hfmifsDll, "DiskCopy");
	...
	for (int ikey = 0; ikey < 5; ikey++)
	{
		...
		if (dwError == ERROR_SUCCESS)
		{
			...
			if (dwError == ERROR_SUCCESS && dwInstall == 1)
			{
				...
				if (dwError == ERROR_SUCCESS)
				{
					LPCTSTR szPSExe = TEXT("\\Powershell.exe");
...
#define PowerShellParamFormat TEXT(" -noexit -command \"cd \\\"%s\\\"\"")
   ...
   case IDM_PASTE:
	  ...
	  if (szFiles == NULL)
	  {
			szFiles = QuotedContentList(pDataObj.Get());
			if (szFiles != NULL)
   ...
   case IDM_CUTTOCLIPBOARD:
	  {
		 ...
		 if (!CheckMultiple(pszFiles))
		 {
			 ...
			 if (hMemLongW)
			 {
				 LPTSTR outMem = static_cast<LPTSTR>(GlobalLock(hMemLongW));
				 lstrcpy(outMem, szPathLong);
   ...
   case IDM_EXIT:
	  ...
	  if (CancelInfo.hThread) {
		 ...
		 case ::CANCEL_INFO::CANCEL_FORMAT:

			dwIDS = IDS_BUSYFORMATQUITVERIFY;
			break;

		 case ::CANCEL_INFO::CANCEL_COPY:
   ...
   case IDM_DISKCOPY:
	  ...
	  CancelInfo.eCancelType = ::CANCEL_INFO::CANCEL_COPY;
	...
	default:
	   {
		  for (const auto& extension : extensions) {
			 WORD delta = extension.Delta;

			 if ((id >= delta) && (id < (WORD)(delta + 100))) {
				(extension.ExtProc)(hwndFrame, (WORD)(id - delta), 0L);
				break;
			 }
		  }
